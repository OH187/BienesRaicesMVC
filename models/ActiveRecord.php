...
public function crear(){
    ...
    if ($resultado) {
        ...
        header("Location: /admin?resultado=1"); //Utilizar poco y solamente cuando no haya codigo html arriba de esta funcion
...
public function actualizar(){
    ...
    if ($resultado) {
        //Redireccionar al usuario despues de ingresar los datos
        header('Location: /admin?resultado=2'); //Utiliar poco y solamente cuando no haya codigo html arriba de esta funcion
...
public function setImagen($imagen){ //set para modificar un valor, es bueno aplicar ese tipo de nombres
    //Elimina la imagen previa
    if(!is_null($this->id)){ //isset() Verifica si existe o que tenga un valor
        $this->borrarImagen();
    }
    //Asignar al atribubto el nombre de la imagen
    if($imagen){
        $this->imagen = $imagen;
    }
}

//Eliminar la imagen del servidor al eliminar los demas datos de la DB
public function borrarImagen(){
     //Comprobar si existe el archivo
        $existeArchivo = file_exists(CARPETAS_IMAGENES . $this->imagen);//Verifica si existe
        if($existeArchivo) {
            unlink(CARPETAS_IMAGENES . $this->imagen); //unlink() elimina el archivo a actualizar
        }
}
